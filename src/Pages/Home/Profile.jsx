      ...
      <div className="HomePage">
        <div className="ProfileDetails">
          ...
          <div className="GithubInfo">
            <Info
              icon={<MdLocationOn />}
              Name={userdatafetch.location || "No location"}
            />
            <Info
              icon={<GiTeacher />}
              Name={userdatafetch.company || "No company"}
            />
            <Info
              icon={<RiGitRepositoryFill />}
              Name={`${userdatafetch.public_repos} Repositories`}
            />
            <Info
              icon={<MdGroups />}
              Name={`${userdatafetch.followers} Followers`}
            />
            <Info
              icon={<MdGroups />}
              Name={`${userdatafetch.following} Following`}
            />
          </div>
        </div>
        <div className="Repositories">
          ...
          <div className="following-container">
            <div className="following-info">
              <h1>Following {userFollowing.length}</h1>
              <div className="following-profile">
                {userFollowing.map((following, i) => (
                  <div key={i} className="following-card">
                    <div className="following-card-img">
                      <img
                        src={following.avatar_url}
                        alt={`Profile picture of ${following.login}`}
                      />
                    </div>
                    <h2>{following.login}</h2>
                    <button>View {following.login}</button>
                  </div>
                ))}
              </div>
            </div>
